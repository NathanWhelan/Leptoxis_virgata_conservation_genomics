...
setwd("~/snails/L_virgata/population-genomics/virgata_m3M3n3_R80_maf025/") ## The directory where the files are located
chunkfile<-"populations.haps_reordered_chunks.out" ## RADpainter output file
mcmcfile<-"populations.haps_reordered_chunks.mcmc.xml" ## finestructure mcmc file
treefile<-"populations.haps_reordered_chunks.mcmcTree.xml" ## finestructure tree file
### 2) EDIT THIS PATH TO WHERE YOU WANT THE PLOTS:
plotsFolder <- "~/snails/L_virgata/population-genomics/virgata_m3M3n3_R80_maf025/"
### 3) SET VALUES FOR THESE VARIABLES: "analysisName" will be included in output plots
analysisName <- "populations.haps_reordered";  maxIndv <- 10000; maxPop<-10000


### 4) EDIT THE PATH TO YOUR COPY of FinestructureLibrary.R
source("~/scripts/FinestructureLibrary.R", chdir = TRUE) # read in the R functions, which also calls the needed packages

### 5) EXECUTE THE CODE ABOVE AND THE REST OF THE CODE BELOW
## make some colours
some.colors<-colorRampPalette(c("#FFFFFF","#FFF3CD","#FDAE6B","#E6550D","#7F0000"))(100) # warm gradient: white-cream-orange-brick-deep red
some.colorsEnd<-c(some.colors, "#4B0000") # as above, but with a deep wine red final for capped values (replaces original near-black)
...
indv.cex.axis<-min(1.1, 80/nrow(dataraw))
...
plotFinestructure(tmpmat,dimnames(tmpmat)[[1]],dend=tdend,cols=some.colorsEnd,cex.axis=indv.cex.axis,edgePar=list(p.lwd=0,t.srt=90,t.off=-0.1,t.cex=1.2))
dev.off()
...
plotFinestructure(tmpmat,dimnames(tmpmat)[[1]],labelsx=labels(popdendclear),labelsatx=labellocs,xcrt=xcrt,cols=some.colorsEnd,ycrt=ycrt,dend=tdend,cex.axis=indv.cex.axis,edgePar=list(p.lwd=0,t.srt=90,t.off=-0.1,t.cex=1.2))
dev.off()
